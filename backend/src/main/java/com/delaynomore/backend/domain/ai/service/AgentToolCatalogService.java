...
public class AgentToolCatalogService {
     ...
     */
    public List<AgentToolResponse> list(Long planId, String owner) {
        return toolRegistry.toolsFor(resolveStatus(planId, owner)).stream()
                .map(AgentToolResponse::from)
                .toList();
    }

    private PlanStatus resolveStatus(Long planId, String owner) {
        if (planId == null) {
            return PlanStatus.DRAFT;
        }
        try {
            PlanResponse plan = planService.getPlan(planId, owner);
            return PlanStatus.fromStored(plan.status());
