...
화면은 **모바일 전용 단일 화면**입니다(v0.18.0) — 하단 고정 탭바로 **대화**(AI 코치와의 대화) · **오늘**("오늘 할 일" — 보관된 계획들의 오늘 항목 모음) · **체크리스트**(생성된 체크리스트)를 오갑니다. 한 번에 한 패널이 세로 전체를 쓰고, 데스크톱에서도 같은 화면이 폰 폭(최대 480px) 컬럼으로 가운데 정렬됩니다.
