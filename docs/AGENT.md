...
### 도구를 부르지 않는 것도 결정이다 [v0.16.2]

인사·감사에 도구를 부르면 권한 모델이 뚫리는 건 아니지만 **왕복과 비용이 늡니다**(3왕복 → 4왕복).
시스템 프롬프트가 사교적 턴을 명시적으로 억제합니다 — 예시(`안녕`, `고마워`, `알겠어`)를 붙여
규칙 목록에서 **별 단락으로 분리**했습니다.

이 문구는 감이 아니라 [평가 결과](QA_RESULT_v0.16.0.md)로 조정한 것입니다. 목록의 마지막 항목으로
뭉쳐 뒀을 때 6회 중 1회 도구를 불렀고, 그 실패가 두 실행 사이에 케이스를 옮겨 다녀 문구 문제가
아니라 성향임이 드러났습니다.

같은 단락에서 **"질문이 섞이면 질문이 이긴다"**(`안녕! 오늘 뭐 해야 해?`)까지 못박습니다. 억제만
강화하면 인사가 붙은 정상 질문에서 도구를 빼먹는 반대쪽 회귀가 생기는데, 그건 사용자에게 더 나쁜
실패입니다(추측한 숫자를 말하게 됨). 평가에도 양쪽 케이스가 함께 있습니다.
...
```
지금 (v0.15.0)   상태 → 도구 집합
다음 (v0.17.0)   상태 → 프로필(시스템 프롬프트 + 도구 집합)
                 DRAFT     → 체크리스트 완성 코치
