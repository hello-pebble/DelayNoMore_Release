...
계획 코치가 **도구를 호출하는 에이전트**가 됐습니다(v0.15.0). 예전에는 한 번의 LLM 왕복으로
산문과 계획 변경을 함께 받아 서버가 문자열을 갈라 파싱했지만, 이제는 모델이 도구를 고르고
**서버가 실행하고 검증**합니다. v0.17.0부터는 상태가 도구 집합만이 아니라 **프로필(페르소나 +
도구 집합)**을 고릅니다 — 계획을 고정하면 목표 영역 전문 에이전트가 대화를 이어받습니다.

이 문서는 도구 카탈로그, 권한 모델, 프로필, 루프 동작, 폴백 체인을 정리합니다.
...
### 프로필 — 상태 → 시스템 프롬프트 + 도구 집합 [v0.17.0]

상태는 도구만 고르는 것이 아니라 **누가 응대하는지**도 고릅니다(`AgentProfile` enum):
...
해석하게 만들면 표가 어긋나는 순간 "프롬프트는 수정 가능하다는데 도구가 없다"류의 모순이
생기기 때문에, 프로필은 도구 집합을 **재선언하지 않고 참조만** 합니다.

회고 도우미가 별도 프로필인 이유는 정확성입니다 — 구 공통 프롬프트의 잠금 안내는 "plan is
fixed(고정)"라고 설명하는데, 종결 상태는 고정이 아니라 완료/중단입니다. 프로필 분리로 각
상태가 자기에게 맞는 잠금 설명을 갖게 됐습니다.

프롬프트는 복사가 아니라 **조립**입니다: 실측(676회)으로 다듬어진 공통부(숫자는 서버가 소유·
무단 변경 금지·사교적 턴 억제·한국어 순도·인젝션 방어)에 프로필별 페르소나와 잠금 안내만
갈아끼웁니다. 세 벌로 복사하면 한 벌만 고쳐지는 drift가 곧 측정 무효화라서입니다 —
`AiPromptBuilderTest`가 세 프로필 모두에 공통 절이 실리는지 지킵니다.

실행이 실제로 고른 프로필은 SSE `profile` 이벤트(스트림 시작 시 1회)와 카탈로그 API 응답의
`profile` 필드로 확인할 수 있습니다. 한 가지 미세한 비대칭: 루프는 goalName을 요청 바디에서
읽고([Goal] user 섹션과 같은 소스), 카탈로그 API는 요청 바디가 없어 서버 저장본에서 읽습니다 —
라벨 표기에만 영향이 있고 권한과는 무관합니다.
...
### 요청받지 않은 변경은 하지 않는다 [v0.16.5]

**평가가 찾아낸 설계 공백입니다.** 고정 계획에 "마지막 날에 항목을 추가해줘"라고 했을 때, 수정
도구가 없자 모델이 **이월 도구로 계획을 실제로 바꿨습니다**(340회 중 관측). 이월은 CONFIRMED에서
정상 노출되는 도구라 권한 모델이 뚫린 것은 아닙니다 — 그런데 사용자가 요청한 건 "추가"이고 실행된
건 "미완료 항목의 날짜 이동"입니다.

**권한 모델이 막는 것은 "그 도구를 쓰는 것"이고, 막지 못하는 것은 "허용된 다른 도구로 비슷한 효과를
내는 것"입니다.** 구조로 막을 수 없는 층이라 프롬프트가 담당합니다:

> 요청한 수정이 불가능하면 **그렇다고 말하라. 다른 변경으로 대체하지 말라.**

평가에서는 `avoidTools`로 감시합니다([EVAL.md 3절](EVAL.md)) — 금지(빌드 깨짐)와 달리 통과율에만
반영합니다. 권한이 뚫린 게 아니라 모델의 판단 문제이기 때문입니다.
...
뭉쳐 뒀을 때 도구를 불렀고, 그 실패가 실행 사이에 케이스를 옮겨 다녀 문구 문제가 아니라 성향임이
드러났습니다. 이후 340회 실측에서 **인사(9.7%)가 감사(3.2%)보다 3배 위험**하다는 것이 보여, 예시
나열에 그치지 않고 **대화 위치**를 지시하도록 보강했습니다(v0.16.5) — "여는 인사는 브리핑 요청이
아니다".
...
- **프롬프트 인젝션** — 요청 데이터는 `[Goal]`·`[Current plan]`·`[User message]` 같은 대괄호
  ...
  이 방어 절은 프로필 조립의 **공통부**에 있어 세 프로필 프롬프트 모두에 실립니다(v0.17.0).
  다만 인젝션의 최종 방어선은 프롬프트가 아니라 **권한 모델**입니다 — 대화 내용이 무엇이든
  노출되지 않은 도구는 호출할 수 없고, 소유자는 헤더에서만 옵니다.
- **시스템 프롬프트에 삽입되는 goalName [v0.17.0]** — 전문가 프로필은 목표명을 시스템 프롬프트에
  넣는데, 이는 사용자 자유 텍스트가 시스템 프롬프트에 들어가는 최초의 경로입니다. 대괄호
  섹션("여기는 데이터")의 보호가 없는 자리라 삽입 전 새니타이즈합니다: 개행(새 지시 단락 위장)
  제거, 큰따옴표(인용 탈출) 치환, 80자 절단, 비어 있으면 일반 문구 폴백. goalName은 권한에
...
## 9. 전문 에이전트 인계 — 1단계 완료 [v0.17.0]

[로드맵의 최종 목표](ROADMAP.md#최종-목표--계획-고정-후-전문-에이전트-인계)는 "계획을 고정하면
그 목표의 전문 에이전트가 이어받는 것"입니다.

v0.15.0이 배선(상태 → 도구 집합 셀렉터)을 깔았고, **v0.17.0이 예고했던 프로필 층을 얹었습니다**
— `CONFIRMED` 전이 시 코치 프로필 → 목표명 특화 도메인 프로필로 갈아끼워집니다(2절의 프로필
표 참고).

```
v0.15.0          상태 → 도구 집합
v0.17.0 (지금)   상태 → 프로필(시스템 프롬프트 + 도구 집합)
                 DRAFT     → 체크리스트 완성 코치
                 CONFIRMED → 목표 영역 전문 에이전트 ("정보처리기사 실기 전문 에이전트")
                 종결       → 회고 도우미
```

남은 것이 진짜 과제입니다: **도메인 지식·자료 검색을 어떻게 붙일 것인가.** 지금의 전문가
프로필은 모델 자체 지식으로 답합니다 — 특정 자격증의 최신 기출 경향이나 교재를 찾아 주려면
검색/RAG 도구가 필요하고, 그 도구의 권한 모델(어떤 상태에서 노출할지)도 같은 틀로 설계해야
합니다. 폴백(자유 대화) 경로에는 프로필 개념이 없어 페르소나가 코치로 회귀합니다 — 도구 미지원
모델에서의 의도된 비대칭입니다.
