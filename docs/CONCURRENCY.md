...
**추가 대상**: 슬랙 일일 발송(v0.26.0) · `SlackDispatchService`의 `@Scheduled` 60초 루프 → 9절
...
목록 조회"라는 점이 위험을 키운다 — 이 경로에는 스케줄러가 없어(v0.26.0 전까지는 저장소 전체에
없었다 — 9절) 만기 챌린지는 누군가 목록을 여는 순간 정산되는데, 목록은 여러 사용자가 동시에 연다.
...
## 9. 같은 문제의 네 번째 판본이자 첫 스케줄러 — 일일 발송은 하루 한 번 (v0.26.0)

슬랙 일일 체크리스트 DM(v0.26.0)이 지키는 불변식: **"한 사용자에게 같은 날 같은 종류의 발송은
최대 한 번."** 지금까지의 주기 작업은 전부 lazy 트리거로 풀었지만 — 정산은 목록 조회 시(8절),
만료 세션·코드 청소는 로그인·발급 시 — 슬랙 푸시는 **사용자의 요청이 없는 시각에 서버가 먼저
보내는** 작업이라 얹혀 갈 요청이 존재하지 않는다. 그래서 저장소 최초의 `@Scheduled`(60초 고정
지연, `SlackDispatchService.dispatch` + `SchedulingConfig`)가 들어왔다.

스케줄러가 들어와도 관통 원칙은 그대로다: **최대 1회의 판정자는 애플리케이션 if가 아니라 쓰기
그 자체다.** 루프는 매분 연결 목록을 훑되, "오늘 이미 보냈는가"를 SELECT로 세어 보지 않고
`slack_daily_sends`의 PK에 클레임을 시도한다:

```sql
INSERT INTO slack_daily_sends (owner, send_date, kind) VALUES (:owner, :date, :kind)
ON CONFLICT (owner, send_date, kind) DO NOTHING   -- 1행 = 발송권 획득, 0행 = 이미 클레임됨
```

정산 `claimSettlement`(8절)과 같은 형태다. 재기동 직후 루프가 다시 돌든, 어떤 이유로 두 실행이
겹치든 — 같은 PK에 INSERT하려는 두 번째는 첫 번째의 커밋 뒤 충돌을 보고 0행으로 물러난다.
중복 DM이 "코드가 조심해서" 없는 게 아니라 **구조적으로** 없다.

전송 실패의 재시도권도 조건부 UPDATE 하나가 준다. 실패한 발송은 `sent_at`이 NULL로 남고,

```sql
UPDATE slack_daily_sends SET attempts = attempts + 1, claimed_at = now()
 WHERE ... AND sent_at IS NULL
   AND claimed_at < now() - interval '5 minutes' AND attempts < 3
```

이 WHERE를 통과해 1행을 받은 실행만 재시도한다 — 둘이 동시에 재클레임해도 행 락 후 WHERE
재평가(2절)로 한쪽만 1행을 받고, 상한 3회를 넘기면 조건이 영구히 거짓이 되어 조용히 마감된다.
전송 성공 확인(`markSent`)이 실패하는 최악의 경우에도 결과는 "최대 3회 재시도로 인한 중복
가능"이지 무한 발송이 아니다.

인메모리 구현은 기존 절들의 대응 관례 그대로, 같은 계약을 맵의 키 단위 원자 구간으로 지킨다 —
클레임은 `putIfAbsent`(null 반환 = 획득), 재클레임은 `computeIfPresent` 안의 검사+증가가 키
단위로 직렬화된다(5절·8절의 규칙). 단일 컨테이너 배포라 인스턴스는 항상 1개지만, 서버를 2대로
늘려도 이 설계는 그대로 안전하다 — 판정이 애플리케이션 메모리가 아니라 DB 행에 있기 때문이다.