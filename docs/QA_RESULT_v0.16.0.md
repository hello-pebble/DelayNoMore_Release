...
1. **프롬프트 조정 → 재측정** — 조정은 v0.16.2에서 했다(사교적 턴 억제를 예시와 함께 별 단락으로
   분리 + 억제 과잉을 감시하는 `read.today.after_greeting` 케이스 추가). **효과는 미측정 상태다.**

   ```bash
   OPENROUTER_API_KEY=... ./gradlew evalAgent -Deval.repeats=5
   ```

   볼 것 셋: ①`notool.*` 2케이스가 5/5로 오르는가 ②`read.today.after_greeting`이 5/5인가(억제가
   과하지 않은가) ③기존 `read.*`가 그대로인가. 케이스가 17개로 늘어 **총계는 위 기준선과 직접
   비교할 수 없다** — 케이스별로 비교한다.
2. **주기 실행 워크플로 + `-Deval.minPassRate`.** 기준선이 두 번 연속 47/48로 확인됐으니 합격선을
   켤 근거가 생겼다. 다만 유일한 실패가 흔들림이므로 임계값은 95% 아래로 잡아야 한다.
3. ~~**리포트의 비용 표기 정리**~~ — 처리됨(v0.16.1). `$0.04795999999999999`처럼 부동소수점 원값이
   새어 나왔던 것을 유효숫자 4자리(`$0.048`)로 고쳤고, `EvalReportTest`로 고정했다.
