...
```json
// 응답 data — PlanResponse 배열. tasks까지 전체를 반환한다(목록에서 진행률 즉시 계산용).
[ { "id": 2, "goalName": "...", "tasks": { }, "savedAt": 1784860000000 },
  { "id": 1, "goalName": "...", "tasks": { }, "savedAt": 1784856600000 } ]
```
...
### 11. PUT /plans/{planId}/reflections/{date} — 오늘 회고 저장 (업서트)
...
### 12. GET /plans/{planId}/reflections/{date} — 특정 날짜 회고 조회

응답 data는 11과 동일. 없으면 404 `REFLECTION_NOT_FOUND`.

### 13. GET /plans/{planId}/reflections — 회고 목록 (날짜 내림차순)
...
### 14. GET /plans/{planId}/audit-events — 이력 조회 (최신순)
...
| `PLAN_UPDATED` | `계획 내용 변경` 또는 이월 감지 문구 |
