...
> [DelayNoMore](https://github.com/hello-pebble/DelayNoMore)의 **"대화를 통해 투두리스트(하루 단위 실행 계획)를 생성하는"** 핵심 흐름만 떼어낸 최소 배포판입니다. 현재 버전: **v0.11.0**
...
- **브라우저별 개인 보관함** — 브라우저가 생성한 게스트 ID로 계획·회고·이력이 격리됩니다(닉네임은 표시용 라벨). 로그인 전 브라우저 단위 임시 보관함이라, 브라우저 데이터 삭제·서버 재시작 시 복구할 수 없습니다.
