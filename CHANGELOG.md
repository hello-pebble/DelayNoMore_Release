...
**측정한 것을 고친다 — 평가 결과로 프롬프트를 조정.** v0.16.0 하네스의 첫 실측
([`docs/QA_RESULT_v0.16.0.md`](docs/QA_RESULT_v0.16.0.md))에서 유일하게 반복된 실패가 **인사·감사에
도구를 부르는 것**이었다(6회 중 1회). 두 번의 독립 실행에서 그 실패가 `notool.greeting` ↔
`notool.thanks` 사이를 옮겨 다녔으므로 **특정 케이스의 문구 문제가 아니라 모델의 성향**이고,
케이스를 다듬을 게 아니라 프롬프트를 고칠 사안이었다. 하네스를 만든 목적("고쳤는데 좋아졌나
나빠졌나")의 첫 실사용이다.
...
```bash
OPENROUTER_API_KEY=... ./gradlew evalAgent -Deval.repeats=5
```

판정 기준은 셋이다 — ①`notool.*` 2케이스가 5/5로 오르는가 ②`read.today.after_greeting`가 5/5인가
(억제 과잉이 없는가) ③기존 `read.*`가 그대로인가. **케이스가 17개로 늘어 총 토큰·비용은 기준선과
직접 비교할 수 없다**(케이스별 값은 비교 가능하다).
