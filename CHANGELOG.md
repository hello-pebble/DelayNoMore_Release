...
닉네임 기반 개인화. 로그인 도입(로드맵 6번) 전 단계로, 닉네임을 비밀번호 없는 간이 계정 키로
도입해 계획·회고·변경 이력을 닉네임별로 격리했다. 같은 닉네임을 입력하면 어느 브라우저에서든
같은 보관함을 본다. 저장소는 계속 휘발성 인메모리(서버 재시작 시 초기화)다.

### Added
- **닉네임 설정 게이트** — 닉네임(트림 후 한글·영문·숫자 2~20자)이 없으면 앱 진입 전에 설정
  화면이 뜬다. 헤더에 현재 닉네임과 "변경" 버튼을 표시하고, 변경하면 그 닉네임의 보관함으로
  전환된다(`nickname.js` + `components/nickname_setup.jsx`, localStorage `delaynomore:nickname`).
- **`X-Nickname` 필수 헤더** — 모든 계획 API(GET 포함)가 퍼센트 인코딩된 닉네임 헤더를 요구한다
  (한글은 HTTP 헤더에 원문으로 실을 수 없어 클라이언트 `encodeURIComponent` → 서버 UTF-8 복원,
  `OwnerNickname` support). 누락은 400 `NICKNAME_REQUIRED`, 규칙 위반은 400 `NICKNAME_INVALID`.
- **소유자 격리** — `Plan`에 `owner` 필드 추가, 목록은 `findAllByOwner`로만 조회. 다른 닉네임의
  계획은 조회·수정·이월·삭제 모두 404 `PLAN_NOT_FOUND`(존재 여부 은닉). 회고·변경 이력은 자체
  소유자 없이 계획의 소유권을 상속하고, 소유자 검사는 저장소의 키 단위 원자 구간 안에서 실행된다.

### Changed
- **(호환성 주의)** 계획 API를 `X-Nickname` 없이 호출하면 400 — curl 등 직접 호출은 헤더를
  추가해야 한다(프론트는 게이트가 항상 보장). 저장소가 휘발성이라 기존 데이터 마이그레이션은
  없고, owner가 없는 레거시 계획은 어떤 조회에도 나타나지 않는다.
- 변경 이력 조회가 계획 소유자 확인을 거치면서, **삭제된 계획의 이력은 더 이상 조회되지 않는다**
  (소유자를 확인할 길이 없으므로 빈 목록 — 이력 자체는 저장소에 남고, UI는 삭제된 계획의 이력을
  조회하지 않아 실사용 영향 없음).
- `X-Session-Id`는 기존 역할 유지 — 같은 닉네임을 여러 브라우저에서 쓸 때 이력의
  "이 브라우저/다른 세션" 구분용.
- 헤더 안내 문구를 "모든 방문자와 공유"에서 "닉네임별로 구분"으로 교체.
