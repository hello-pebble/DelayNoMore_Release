...
- 봇 답변에 **타이핑 효과** — 채팅 말풍선이 한 번에 나타나지 않고 점진적으로 드러나
  스트리밍처럼 보인다(약 60자/초 + 깜빡이는 커서). 말풍선 클릭 시 즉시 전체 텍스트로
  스킵 가능. (최초 구현은 약 190자/초라 사실상 즉시 표시로 보여 속도를 낮춤.
  실제 토큰 스트리밍(SSE)은 응답이 JSON 계약이라 증분 파싱이 필요해 추후 과제로 분리.)
